...
func WriteDocsMarkdown(r chi.Router) {
...
func WriteDocsJSON(r chi.Router) {

	// Create a new `routes.md` file.
	if err := os.Remove("./docs/routes.json"); err != nil && !errors.Is(err, os.ErrNotExist) {
		log.Fatal(err)
	}
	f, err := os.Create("./docs/routes.json")
